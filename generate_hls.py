...
def get_video_info(video_path):
    """Mendapatkan resolusi video input dan keberadaan audio menggunakan ffprobe."""
    cmd = [
        "ffprobe",
        "-v", "error",
    ...
    try:
        ...
        for st in streams:
            if st.get("codec_type") == "video":
                width = st.get("width", width)
                height = st.get("height", height)
...
def update_library_metadata(workspace_dir):
    ...
    IGNORED_DIRS = {"RAW", "RAW_TEMP", "RAW_TEMP_EXTRACT", "release_downloads", ".git", "assets", ".vscode", ".gemini", "__pycache__"}
...
def process_single_video(video_path):
    ...
    # Dapatkan resolusi asli video, keberadaan audio, & stream subtitle
    in_w, in_h, has_audio = get_video_info(video_path)
    subtitles = get_subtitle_streams(video_path)
    print(f"[+] Deteksi resolusi input: {in_w}x{in_h} (Audio: {'Ada' if has_audio else 'Tidak ada/Bisu'})")
    if subtitles:
        print(f"[+] Ditemukan {len(subtitles)} stream subtitle.")
    ...
    for i, p in enumerate(filtered_profiles):
        ...
        # Map audio jika video memiliki stream audio
        if has_audio:
            ffmpeg_cmd.extend([
                "-map", "0:a:0?",
                f"-c:a:{i}", "aac",
